...
void staircase_clock_drawHours(uint8_t hour) {
    ...
    microGL_draw3x5Digit(0, 0, hour / 10, palette_getColor(COLOR_A));
    microGL_draw3x5Digit(3, 2, hour % 10, palette_getColor(COLOR_B));
}

void staircase_clock_drawMinutes(uint8_t minute) {
    microGL_draw3x5Digit(6, 5, minute / 10, palette_getColor(COLOR_C));
    microGL_draw3x5Digit(9, 7, minute % 10, palette_getColor(COLOR_D));
}